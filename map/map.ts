...
export class MapPage {
  ...
  constructor(private platform: Platform, private events: Events, private app: App,
    private geolocation: Geolocation, private alertCtrl: AlertController, private filters: FilterService) {
    platform.ready().then(() => {
      this.db = new SQLite();
  ...
  loadArbres() {
    ...
    this.db.executeSql(select_arbres, []).then((data) => {
      this.arbres = [];
  ...
  openDetail(id: number) {
    ...
    this.db.executeSql(select_essence, []).then((data) => {
      if (data.rows.length > 0) {
